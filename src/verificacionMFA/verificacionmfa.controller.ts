import { Controller, Post, Get, Body, Headers, UseGuards, Query, VersioningType } from "@nestjs/common";
...
export class VerificacionmfaController {
    ...
    @UseGuards(AuthGuard)
    async nuevaVerificacionMFA(@Headers("authorization") authHeader: string,
        @Body() verificacion: VerificacionMFA) {
        try {
            const usuario = await this.authInstance.getUserByUid(verificacion.uid);
            this.email.enviarEmail(usuario.email, `Utiliza este codigo para poder ver tu perfil: ${verificacion.codigo}`, "Tu codigo de Verificación")
            return {
                ok: true,
                data: await this.verificacionInstance.nuevaVerificacionMFA(verificacion)
            };
    ...
    async verificacionMFA(
        ...
        try {
            ...
            const respVerificacion = await this.verificacionInstance.verificacionMFA(uid, bol)
            return {
                ok: true,
