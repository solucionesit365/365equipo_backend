...
describe('GetTiendasAteneaController', () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<GetTiendasAteneaController>(GetTiendasAteneaController);
    useCase = module.get(IGetTiendasAteneaUseCase) as jest.Mocked<IGetTiendasAteneaUseCase>;
  });
  ...
  describe('handleForAtenea', () => {
    ...
    it('debe propagar errores del use case', async () => {
      ...
      await expect(controller.handleForAtenea()).rejects.toThrow('Database error');
      expect(useCase.execute).toHaveBeenCalled();
    });
  });
